...
export type BannerDisplayVariant =
  'danger' |
  'info' |
  'success' |
  'warning'
...
export const layoutStore = createStore<LayoutState>((set, get) => ({
  ...
  space: undefined,
  team: undefined,
  hideBackdrop: () => set((state: LayoutState) => ({ backdrop: { open: false, message: undefined } })),
  hideScreenshot: () => set((state: LayoutState) => ({ screenshot: { open: false, message: undefined } })),
  setBootstrapped: (bootstrapped: boolean) => set((state: LayoutState) => ({ bootstrapped })),
  showBackdrop: (message?: string) => set((state: LayoutState) => ({ backdrop: { open: true, message } })),
  showScreenshot: (message?: string) => set((state: LayoutState) => ({ screenshot: { open: true, message } })),
  setBanner: (bannerDisplay: BannerDisplay) => set((state: LayoutState) => ({ 
    banner: {
      timestamp: new Date(),
  ...
  setLeftPortal: (leftPortal: PortalDisplay) => set((state: LayoutState) => ({ leftPortal })),
  setRightPortal: (rightPortal: PortalDisplay) => set((state: LayoutState) => ({ rightPortal })),
  resetLeftPortal: () => set((state: LayoutState) => ({ leftPortal: state.leftPortal?.pinned ? state.leftPortal : undefined, })),
  resetRightPortal: () => set((state: LayoutState) => ({ rightPortal: state.rightPortal?.pinned ? state.rightPortal : undefined, })),
  resetForcedLeftPortal: () => set((state: LayoutState) => ({ leftPortal: undefined })),
  resetForcedRightPortal: () => set((state: LayoutState) => ({ rightPortal: undefined })),
  setLeftSidebarVariant: (leftSidebarVariant: LeftSidebarVariant) => set((state: LayoutState) => ({ leftSidebarVariant })),
  updateLayoutOrganization: (organization?: Partial<IAnyOrganization>) => set((state: LayoutState) => {
    return ({
      organization:
        organization ? {
          ...state.organization,
  ...
  updateLayoutTeam: (team?: Partial<IAnyTeam>) => set((state: LayoutState) => {
    return ({
      team:
      team ? {
          ...state.team,
  ...
  updateLayoutSpace: (space?: Partial<IAnySpace>) => set((state: LayoutState) => {
    return ({
      space:
        space ? {
          ...state.space,
  ...
  triggerItemsRefresh: () => set((state: LayoutState) => ({ itemsRefreshCount: state.itemsRefreshCount + 1 })),
  setScreenCapture: (screenCapture?: string) => set((state: LayoutState) => ({ screenCapture })),
