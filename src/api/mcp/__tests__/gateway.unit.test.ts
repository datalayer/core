...
describe('MCP gateway addressing', () => {
  it('takes the resource path off the configured MCP URL', () => {
    expect(mcpGatewayOrigin('https://r1.datalayer.run/mcp')).toBe(
  ...
  it('builds versioned routes with only the query parameters that have a value', () => {
    expect(
      mcpUrl('https://r1.datalayer.run/mcp', '/tasks', {
        status: 'working',
        cursor: undefined,
        limit: 20,
        org: '',
      }),
    ).toBe('https://r1.datalayer.run/api/mcp/v1/tasks?status=working&limit=20');
    expect(mcpServiceUrl('https://r1.datalayer.run/mcp', '/version')).toBe(
      'https://r1.datalayer.run/api/mcp/version',
    );
  });
});
...
describe('MCP wire conversion', () => {
  ...
  it("keeps redacted arguments and client info under the tool's own names", () => {
    ...
    expect(row.redactedArguments).toEqual({
