...
class Runtime(DatalayerClientAuthMixin, RuntimesMixin, SnapshotsMixin):
    ...
    def execute_file(
        ...
        """
        fname = Path(file_path).expanduser().resolve()
        ...
        silent = False
        timeout = self._timeout * 60  # Convert minutes to seconds
        ...
        #         raise
        #     self.log.warning("Unknown failure executing file: <%s>", fname, exc_info=e)
        # finally:
        #     self._executing = False
        return Response([])
    ...
    def execute_notebook(
        self, notebook_path: str, variables: Optional[dict[str, Any]] = None
    ) -> Response:
        """
        Execute a Jupyter notebook in the runtime.

        Parameters
        ----------
        notebook_path: str
            Path to the Jupyter notebook file to execute.
        variables: Optional[dict[str, Any]]
            Optional variables to set before executing the code.

        Returns
        -------
            Response: The result of the notebook execution.
        """
        return Response([])
