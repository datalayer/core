...
class Client(DatalayerClient):
    urls = property(
        lambda self: SimpleNamespace(contents_url="https://contents.example")
    )
    ...
    def list_content_sources(self, **kwargs: Any) -> Any:
...
MANIFEST = {
    "src-1": {"tools": [{"name": "search"}, {"name": "read_file"}]},
    "src-3": {"tools": []},
}


class TestTheCatalogue:
    def test_only_sources_that_lend_tools(self) -> None:
    ...
    def test_a_source_whose_manifest_fails_is_left_out_not_fatal(self) -> None:
        client = Client(sources=[SOURCE], manifests={})
        assert client.list_toolsets() == []


class TestEnablingAndDisabling:
    def test_enable_posts_a_session_with_an_idempotency_key(self) -> None:
    ...
    def test_disable_deletes_the_session(self) -> None:
        ...
        assert (method, url) == (
            "DELETE",
            "https://contents.example/api/contents/v1/mcp-sessions/sess-1",
        )
...
class TestTheCommands:
    def test_the_three_exist(self) -> None:
        assert {command.name for command in cli.toolsets_app.registered_commands} == {
            "list",
            "enable",
            "disable",
        }

    def test_list_says_which_are_enabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        client = Client(
            sources=[SOURCE],
            manifests=MANIFEST,
            sessions=[
                {"uid": "sess-1", "source_uid": "src-1", "status": "active"},
        ...
        assert "sess-0" not in answer.output, (
            "a revoked session is not an enabled toolset"
        )
        assert "read_file, search" in answer.output

    def test_list_when_nothing_lends_tools(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            cli, "_client", lambda: Client(sources=[QUIET], manifests={})
        )
        answer = CliRunner().invoke(cli.app, ["toolsets", "list"])
        assert (
            answer.exit_code == 0 and "No source of yours lends tools" in answer.output
        )

    def test_enable_prints_what_was_allowed(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(
            cli,
        ...
        answer = CliRunner().invoke(
            cli.app, ["toolsets", "enable", "src-1", "--tool", "search"]
        )
        assert answer.exit_code == 0, answer.output
        assert (
            "sess-1" in answer.output
            and "search" in answer.output
            and "explicit" in answer.output
        )

    def test_disable_asks_before_revoking(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        client = Client(answer={"uid": "sess-1", "status": "revoked"})
        monkeypatch.setattr(cli, "_client", lambda: client)
        refused = CliRunner().invoke(
            cli.app, ["toolsets", "disable", "sess-1"], input="n\n"
        )
