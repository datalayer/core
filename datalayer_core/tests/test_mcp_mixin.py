...
class Recorder(McpMixin):
    ...
    def _iam_url(self, path: str = "") -> str:
        return f"https://iam.test/api/iam/v1{path}"

    def _mcp_url(self, path: str = "", **parameters: Any) -> str:
        return f"https://mcp.test/api/mcp/v1{path}"

    def _fetch(
        self, url: str, method: str = "GET", json: Any = None, **kwargs: Any
    ) -> Any:
        self.sent = {"url": url, "method": method, "json": json}
        return SimpleNamespace(
            json=lambda: {"settings": {}, "success": True}, content=b"{}"
        )
...
class TestAlertDestinations:
    def test_only_what_was_passed_is_sent(self, sdk: Recorder) -> None:
    ...
    def test_a_cleared_destination_is_sent_rather_than_dropped(
        self, sdk: Recorder
    ) -> None:
    ...
    def test_slack_is_its_own_field(self, sdk: Recorder) -> None:
        sdk.set_mcp_alert_destinations("01ORG", slack="https://hooks.slack.com/x")
        assert sdk.sent["json"] == {
            "alert_slack_webhook_url": "https://hooks.slack.com/x"
        }

    def test_all_three_at_once(self, sdk: Recorder) -> None:
...
class TestThePolicyLayer:
    def test_a_write_carries_the_version_as_a_query(self, sdk: Recorder) -> None:
        sdk.set_mcp_policy_layer(
            "organization", "01ORG", {"maxCallsPerMinute": 30}, expected_version=7
        )
        assert "expected_version=7" in sdk.sent["url"]

    def test_a_version_of_zero_is_carried_rather_than_dropped(
        self, sdk: Recorder
    ) -> None:
    ...
    def test_no_version_sends_none(self, sdk: Recorder) -> None:
    ...
    def test_the_rules_are_the_body(self, sdk: Recorder) -> None:
        sdk.set_mcp_policy_layer(
            "personal", "01USER", {"toolDenylist": ["execute_cell"]}
        )
        assert sdk.sent["json"] == {"toolDenylist": ["execute_cell"]}

    def test_a_layer_is_addressed_by_scope_and_subject(self, sdk: Recorder) -> None:
        sdk.set_mcp_policy_layer("team", "01TEAM", {})
        assert "/mcp-policies/team/01TEAM" in sdk.sent["url"]


class TestTryingARule:
    def test_it_asks_the_gateway_not_iam(self, sdk: Recorder) -> None:
