...
class TestListing:
    def test_it_asks_the_versions_route(self) -> None:
        client = Client({"success": True, "versions": [{"uid": "v1"}, {"uid": "v0"}]})
        assert [v["uid"] for v in client.list_notebook_versions("ntb-1")] == [
            "v1",
            "v0",
        ]
    ...
    def test_a_failure_is_an_empty_list(self) -> None:
        assert Client(RuntimeError("down")).list_notebook_versions("ntb-1") == []


class TestSnapshot:
    def test_it_posts_the_message(self) -> None:
        client = Client(
            {"success": True, "version": {"uid": "v9", "message": "before"}}
        )
    ...
    def test_a_refusal_is_an_error_in_words(self) -> None:
        client = Client(
            {
                "success": False,
                "detail": "Not authorized to access this notebook's versions.",
            }
        )
...
class TestRestore:
    def test_it_posts_to_the_versions_restore_route(self) -> None:
        client = Client(
            {
                "success": True,
                "restored": {"uid": "v1"},
                "kept": {"uid": "v2", "reason": "restore"},
            }
        )
        ...
        url, kwargs = client.calls[0]
        assert (
            url.endswith("/notebooks/ntb-1/versions/v1/restore")
    ...
    def test_an_unknown_version(self) -> None:
        with pytest.raises(RuntimeError, match="Version not found"):
            Client(
                {"success": False, "detail": "Version not found."}
            ).restore_notebook_version("ntb-1", "nope")
