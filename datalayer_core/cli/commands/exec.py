...
class RuntimesExecService:
    ...
    def handle_sigint(self, *args) -> None:
        ...
        if self._executing:
            if self.kernel_manager:
                ...
                console.print("[red]ERROR: Cannot interrupt kernels we didn't start.[/red]", file=sys.stderr)
        else:
    ...
    def init_kernel_manager(self, runtime_name: str) -> None:
        ...
        try:
            ...
            console.print(f"[red]Failed to connect to runtime '{runtime_name}': {e}[/red]")
            
            # Provide helpful authentication guidance
            if "Token is required" in str(e) or "authentication" in str(e).lower():
                console.print("[yellow]Hint: Make sure you're authenticated. You can:[/yellow]")
                console.print("[yellow]  1. Run 'dla login' to authenticate[/yellow]")
                console.print("[yellow]  2. Set DATALAYER_API_KEY environment variable[/yellow]")
                ...
                try:
                    ...
                    if raise_exceptions and reply["content"]["status"] != "ok":
                        ...
                        if content["status"] == "error":
                            if cell_id:
                                console.print(f"[red]Exception when running cell {cell_id}[/red]")
                            console.print("[red]" + "\n".join(content["traceback"]) + "[/red]")
                            raise typer.Exit(1)
                        else:
                            raise RuntimeError(f"Unknown failure: {json.dumps(content)}")
                    
                    # Show success for each cell if not silent
                    if not silent:
                        status = reply["content"]["status"]
                        if status == "ok":
                            console.print(f"[green]✓ Cell {i} executed successfully[/green]")
                        else:
                            console.print(f"[yellow]⚠ Cell {i} completed with status: {status}[/yellow]")
...
def main(
    ...
    runtime: Optional[str] = typer.Option(None, "--runtime", "-r", help="Name of the runtime to execute on (uses first available if not specified)"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show all cell outputs"),
    timeout: Optional[float] = typer.Option(None, "--timeout", "-t", help="Execution timeout for each cell in seconds"),
    raise_exceptions: bool = typer.Option(False, "--raise", help="Stop executing if an exception occurs"),
) -> None:
    ...
    try:
        ...
        console.print(f"[red]Error: Could not open file '{filepath}' for reading: {e}[/red]")
        raise typer.Exit(1)
    
    # Check file extension
    if filepath.suffix not in ['.py', '.ipynb']:
        console.print(f"[yellow]Warning: File extension '{filepath.suffix}' is not .py or .ipynb[/yellow]")
...
def _select_runtime() -> str:
    ...
    try:
        ...
        selected = runtimes[0]
        console.print(f"[blue]No runtime specified, using: {selected.name} ({selected.uid})[/blue]")
        ...
        console.print("[yellow]Hint: Make sure you're authenticated with 'dla login'[/yellow]")
        raise typer.Exit(1)
