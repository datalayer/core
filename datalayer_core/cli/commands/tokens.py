...
def create_token(
    ...
    try:
        ...
        if result.get("success", False):
            ...
            console.print(f"[yellow]Token value: {result.get('access_token', 'N/A')}[/yellow]")
            console.print("[dim]Please save this token value securely - it won't be shown again![/dim]")
            ...
            if token_data:
                display_tokens([{
                    ...
                    'description_t': token_data.get('description_t', description),
                    'variant_s': token_data.get('variant_s', token_type),
                }])
        else:
            console.print(f"[red]Failed to create token: {result.get('message', 'Unknown error')}[/red]")
            raise typer.Exit(1)
