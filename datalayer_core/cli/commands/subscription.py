...
def _render_available_plans(plans: list[dict[str, Any]]) -> None:
    ...
    for plan in plans:
        ...
        interval = (
            plan.get('interval') or recurring.get('interval') or plan.get('billing_period')
        )
        plans_table.add_row(
            _normalize_value(
                plan.get('name')
            ...
            _normalize_value(
                plan.get('included_runs') or (plan.get('metadata') or {}).get('included_runs')
            ),
        )
...
def subscription_topups(
    ...
    try:
        ...
        has_topup_access = status in {'active', 'trialing'} and str(plan_name).lower() not in {
            'free',
            'none',
        }
...
def subscription_dry_run(
    ...
    try:
        ...
        console.print('This walkthrough validates Stripe test configuration end-to-end.')
        ...
        if check_api:
            ...
            if create_intent:
                ...
                if price_id:
                    ...
                    if intent_resp.get('success', True) and intent_resp.get('client_secret'):
                        console.print(
        ...
        console.print(cards)
        console.print('Use any future expiry date, any CVC, and any postal code in test mode.')
