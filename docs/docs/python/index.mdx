...
A simple, intuitive Client for AI engineers to work with the [Datalayer platform](https://datalayer.app/).

`datalayer-core` provides the **account and platform** client: authentication, secrets,
API keys, usage, and profile. **Runtime creation and code execution** live in the
[`agent-runtimes`](https://github.com/datalayer/agent-runtimes) package via its
`RuntimeClient` (used throughout the runtime examples below).

## Features

- **Simple Authentication**: Easy token-based authentication with environment variable support
- **Account & Platform**: Secrets, API keys, usage, and profile via `datalayer-core`
- **Runtime Management**: Create and manage compute runtimes (CPU/GPU) via `agent-runtimes` `RuntimeClient`
...
```bash
pip install datalayer_core   # account/platform client
pip install agent-runtimes    # runtime creation and code execution
```
...
client = DatalayerClient(api_key="your-token-here")
...
This Client is designed to be simple and extensible. Feel free to submit [issues](https://github.com/datalayer/core/issues) and enhancement requests!

## License

This Client is open source software licensed under the [BSD 3-Clause License](https://github.com/datalayer/core/blob/main/LICENSE).
